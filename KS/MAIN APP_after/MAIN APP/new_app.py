...
from name_extractor import crawl_names
...
def main():
	...
	while (inp != 'q'):
		...
		dols = extract_dol(body)  # extract DOL
		...
		# adding the two items in the body_fetched list
		...
		for item in sign_items_post:
			if item in req_line.upper().split():
				req_item = item
				break

		print 'NO OF CASES/SIGNS: ', req_item
		print '\n' 
		names = crawl_names(remaining_body_name)  # extract the names from the remaining body
		names_copy = [x.upper() for x in names]  # create a copy to iterate
		print '\n'

		
		print 'Name: ',
		if len(names_copy) != 0:  # if there are names extracted
			if 'DAVID' in names_copy[0]:  # if first line contains DAVID ignore it.
				names.remove(names[0]) 
			unwanted_names = []
			for neme in names:
				if 'REGARDS' in neme.upper():
					unwanted_names.append(neme)
				if 'GAURAV' in neme.upper():
					unwanted_names.append(neme)
				if 'SHARMA' in neme.upper():
					unwanted_names.append(neme)

			for unwanted_name in unwanted_names:
				try:
					names.remove(neme)
				except:
					pass
		try:
			names = process_names(names)  # remove the stopwords
		except:
			pass
